...
for _ in range(2):  # 2 experiments from FAA. 
    for cv_fold in range(cv_num): # For the 5 folds. 
        ...
        import pickle
        savefp = os.path.join(args.policy_dir, str(args.dataid + '_MAX_' + args.base+'_'+args.loss+ '_' + (str(reward_attr) + '.pkl')))
...
import pickle
savefp = os.path.join(args.policy_dir, str(args.dataid + '_' + args.base+'_'+args.loss+ '_' + (str(reward_attr) + '.pkl')))
with open(savefp, 'wb') as f: 
    pickle.dump(final_policy_set, f)